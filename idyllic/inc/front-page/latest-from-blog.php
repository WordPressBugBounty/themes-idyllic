			...
			<div class="wrap">
				...
				if($idyllic_settings['idyllic_latest_blog_title'] != '' || $idyllic_settings['idyllic_latest_blog_description'] != ''){ ?>
					...
					<?php if($idyllic_settings['idyllic_latest_blog_title'] != '') { ?>
						<h2 class="box-title freesia-animation zoomIn" data-wow-delay="0.3s"><?php echo esc_html($idyllic_settings['idyllic_latest_blog_title']);?> </h2>
					<?php }
					if($idyllic_settings['idyllic_latest_blog_description'] != ''){ ?>
						<p class="box-sub-title freesia-animation zoomIn" data-wow-delay="0.3s"><?php echo esc_html($idyllic_settings['idyllic_latest_blog_description']);?></p>
					<?php } ?>
					</div><!-- end .box-header -->
				<?php } ?>
						...
						<article id="post-<?php the_ID(); ?>" <?php post_class();?>>
							...
							<div class="latest-blog-text">
								<header class="entry-header">
									...
									<div class="entry-meta">
										<span class="author vcard"><?php esc_html_e('Post By','idyllic');?><a href="<?php echo esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )); ?>" title="<?php the_title_attribute(); ?>"><?php the_author(); ?></a></span>
								...
								<div class="entry-content">
									<?php the_content(esc_html($idyllic_settings['idyllic_tag_text'])); ?>
