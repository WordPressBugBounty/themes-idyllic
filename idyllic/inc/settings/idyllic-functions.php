			...
			$excerpt = get_the_excerpt();
				...
				if($idyllic_settings['idyllic_slider_button'] == 0){
					...
					if(!empty($slider_custom_text)){
						$idyllic_category_sliders_display 	.= '<a title="'.esc_attr($slider_custom_text).'"' .' href="'.esc_url($slider_custom_url). '"'. ' class="btn-default vivid-blue" target="_blank">'.esc_html($slider_custom_text). '</a>';
					}
...
function idyllic_scripts() {
	...
	wp_register_style( 'idyllic-google-fonts', wptt_get_webfont_url('https://fonts.googleapis.com/css?family='.$idyllic_googlefonts));
